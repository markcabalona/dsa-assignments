...
void getRecs();//prompts user to enter their infos MAX times
void aver();//computes the average of of each user
void display();//displays the infos
...
void aver(){
    for(int i = 0; i< MAX; i++){//computes average MAX times
...
void display()
{
    ...
    {
        printf("%d  %.8s\t%.2f\t%.2f\t%.2f\t%.2f\t%s\n", i + 1, name[i], g1[i], g2[i], g3[i], average[i], isPassed[i] == 1 ? "Passed" : "Failed");
    }
    printf("\n\n\t\tYUN OH!\n");
    //pauses the execution of the program until a key is pressed
    system("pause");
...
char *get_string(char *prompt)
{
    ...
    printf("%s", prompt);
    scanf("%[^\n]", temp);//accepts string input until \n
    getchar();//flushes the \n character 
